...
## Bootstrapping — if `cdp list` fails

If `cdp list` errors with "No Chrome/Chromium binary found" or similar, Chrome is either not running or not running with remote debugging. **Do not fall back to curl.** Instead, launch Chrome yourself:

```bash
google-chrome --remote-debugging-port=9222 &
sleep 2          # give it a moment to start
cdp list         # should now work
```

If Chrome is already running **without** remote debugging, you must restart it:
...
> **Note:** Restarting Chrome will close the user's existing tabs (they'll be restored on next launch). Warn the user before doing this.

Once `cdp list` succeeds, proceed normally with `cdp open`, `cdp nav`, etc.
