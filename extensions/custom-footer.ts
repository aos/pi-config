			...
			return {
				...
				render(width: number): string[] {
					...
					if (visibleWidth(pwd) > maxPwdW) {
						...
						if (half > 1) pwd = `${pwd.slice(0, half)}...${pwd.slice(-(half - 1))}`;
					...
					const lastMsg =
						lastAssistant?.type === "message" ? (lastAssistant.message as AssistantMessage) : null;
					const ctxTokens = lastMsg
						? lastMsg.usage.input + lastMsg.usage.output + lastMsg.usage.cacheRead + lastMsg.usage.cacheWrite
						: 0;
					...
					if (footerData.getAvailableProviderCount() > 1 && ctx.model) {
						...
						if (statsLeftW + 2 + visibleWidth(withProv) <= width) right = withProv;
					}

					const rightW = visibleWidth(right);
					let statsLine: string;
					if (statsLeftW + 2 + rightW <= width) {
						statsLine = statsLeft + " ".repeat(width - statsLeftW - rightW) + right;
					} else {
						const avail = width - statsLeftW - 2;
						if (avail > 3) {
							const tr = right.replace(/\x1b\[[0-9;]*m/g, "").slice(0, avail);
							statsLine = statsLeft + " ".repeat(width - statsLeftW - tr.length) + tr;
						} else {
							statsLine = statsLeft;
						}
					}
					...
					if (statusW > 0 && pwdW + 2 + statusW <= width) {
						...
						pwdLine = theme.fg("dim", pwd) + theme.fg("dim", pad) + theme.fg("dim", statusStr);
					} else if (statusW > 0) {
						...
						if (availStatus > 3) {
							...
							pwdLine = theme.fg("dim", pwd) + theme.fg("dim", pad + truncStatus);
						} else {
