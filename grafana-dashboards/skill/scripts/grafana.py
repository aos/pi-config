...
CDP_SCRIPT = (
    Path(__file__).resolve().parent.parent.parent / "chrome-cdp" / "scripts" / "cdp.mjs"
)
...
def _fetch_js(
    path: str, org_id: str, method: str = "GET", body: dict | None = None
) -> str:
    """Build a JS async fetch snippet with error handling."""
    headers = json.dumps(
        {"Content-Type": "application/json", "X-Grafana-Org-Id": org_id}
    )
    body_part = ""
    if body is not None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Grafana API operations via chrome-cdp"
    )
    parser.add_argument(
        "--target",
        required=True,
        help="Chrome CDP target ID prefix (from cdp.mjs list)",
    )
    ...
    p_query.add_argument(
        "--range", help="Time range (e.g. 30d, 24h, 5m). Omit for instant query"
    )
    p_query.add_argument(
        "--step",
        help="Step interval for range queries (e.g. 1h, 5m). Auto-calculated if omitted",
    )
    ...
    p_inspect = sub.add_parser(
        "inspect-panels", help="Show panel summary (id, title, type, position)"
    )
